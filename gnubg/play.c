...
#if USE_GTK
static unsigned int nTimeout, fDelaying;
int fEndDelay;
...
//			GTKDelay();
    ...
    if ( GameStatus( ms.anBoard, ms.bgv )  || ms.gs == GAME_DROP || ms.gs == GAME_RESIGNED)
	{
		moverecord *pmr = (moverecord *) plGame->plNext->p;
		xmovegameinfo *pmgi = &pmr->g;
		...
		outputf( ngettext( "%s wins a %s and %d point.\n", "%s wins a %s and %d points.\n", pmgi->nPoints ),
				ap[ pmgi->fWinner ].szName, gettext( aszGameResult[ n - 1 ] ), pmgi->nPoints);
